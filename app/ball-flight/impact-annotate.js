...
// D76/D63: all vinkel->retning-dekomponering av MOTORVINKLER leveres av
// adapteren (fysikklint kat 2). Aritmetikk paa komponentene er var egen.
...
function buildArc(basis, gen, angDeg, steps = 18) {
  ...
  // gen mottar GRADER og henter retningsvektoren fra adapteren (D63).
  const angMagDeg = Math.abs(angDeg);
  const pts = [];
  for (let i = 0; i <= steps; i++) {
    const aDeg = (angMagDeg * i) / steps;
    const q = project(gen(aDeg, 1), basis);
    if (q) pts.push(q);
  }
  ...
  const labelAnchor = project(gen(angMagDeg * 0.5, 1.3), basis);
...
export function buildAnnotations(outcome, station, basis, hotKey = null) {
  ...
  const [dirS, dirC] = directionRay(launchDir);
  const tanDir = dirS / dirC;
  ...
  if (s < 1.4) {
    ...
    if (best) {
      primitives.push({
        ...
        label: s < 1.35 ? `Apex ${apex.toFixed(1)} m` : null, // D28: 1 des
  ...
  if (topA > 0.03) {
    ...
    const dirArc = buildArc(basis,
      (aDeg, m) => {
        const [s_, c_] = directionRay(aDeg);
        return { x: c_ * 26 * m, y: s_ * 26 * m * sign(launchDir), z: 0 };
      },
      launchDir);
    if (dirArc) {
      primitives.push({
    ...
    if (Math.abs(curve) >= 3) {
      ...
      if (a && b) primitives.push({
        ...
        label: `${Math.abs(curve).toFixed(1)} m curve`, // D28
      });
    }
    ...
    // offline-brakett ved z = carry + 12
    {
      ...
      if (a && b) primitives.push({
        ...
        label: `${Math.abs(side).toFixed(1)} m ${side >= 0 ? 'R' : 'L'}`, // D28/D29
      });
    }
  }
  ...
  if (sideA > 0.03) {
    ...
    if (landingEnd) primitives.push({
      ...
      label: `Landing Angle ${landAng.toFixed(1)}°`, // D28
...
export function placeLabels(primitives, keepOut, vbox) {
  ...
  for (const prim of primitives) {
    ...
    // labelBase: ankeret foer kaskaden — D18-hysteresen i tegnelaget maaler
    // OFFSET (pos - base), slik at scenebevegelse foelges instant mens
    // slotbytter faar 120/300 ms-terskler.
    out.push({ ...prim, labelPos: placed, labelBase: base });
  }
  return out;
}
