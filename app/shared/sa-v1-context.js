// D76-ommaling: gammel motor byttet til engine/ (drop-in — samme signatur),
// og yard->meter gaar via adapterens konverteringslag (D58: ETT sted).
...
const toMetres = yd => distanceForDisplay(yd, 'meters').value;
...
function relationshipFor(engine) {
  ...
  const curve = sideLabel(toMetres(engine.curve), 0.75);
...
function resultForInputs(inputs) {
  ...
  return {
    shape: engine.shape,
    carryM: roundOne(toMetres(engine.carry)),
    totalM: roundOne(toMetres(engine.total)),
    offlineM: roundOne(toMetres(engine.offline)),
    curveM: roundOne(toMetres(engine.curve)),
    ...
    curveLabel: sideLabel(toMetres(engine.curve), 0.75),
    finishLabel: sideLabel(toMetres(engine.offline), 0.75),
